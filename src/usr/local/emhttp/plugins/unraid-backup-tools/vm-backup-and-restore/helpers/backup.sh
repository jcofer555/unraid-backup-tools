...
STOP_FLAG="/tmp/unraid-backup-tools/vm-backup-and-restore/stop_requested.txt"
RSYNC_PID=""
...
run_rsync() {
    ...
    rsync "$@" &
    RSYNC_PID=$!
    echo "$RSYNC_PID" > "/tmp/unraid-backup-tools/vm-backup-and-restore/rsync.pid"
    wait $RSYNC_PID
    local exit_code=$?
    RSYNC_PID=""
    rm -f "/tmp/unraid-backup-tools/vm-backup-and-restore/rsync.pid"
...
sed -i "s/PID=.*/PID=$$/" "/tmp/unraid-backup-tools/vm-backup-and-restore/lock.txt"
...
CONFIG="/boot/config/plugins/unraid-backup-tools/vm-backup-and-restore/settings.cfg"
